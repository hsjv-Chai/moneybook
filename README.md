...
- **微信账单导入**：直接导入微信支付账单（xlsx / CSV / 由账单生成的 PDF），自动建立账户、按商户关键词分类、跳过重复记录与疑似重复提示，并可整批撤销（见下节）。
...
| 来源 | 说明 |
| --- | --- |
| 微信账单 xlsx | 微信「账单流水文件」导出（当前版本的默认格式）。用系统 Compression 解 ZIP、再解 XML 得到单元格，数据精确。 |
| 微信账单 CSV | 微信「账单下载 → 用于个人对账」邮箱收到的 CSV，按列名解析，数据精确。 |
